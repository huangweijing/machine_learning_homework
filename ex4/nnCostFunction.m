...
##微分和应当和连接线数（Theta）保持一致
delta_sum2 = zeros(num_labels, hidden_layer_size + 1);
delta_sum1 = zeros(hidden_layer_size, input_layer_size + 1);

##对于m个测试数据，求出每一个节点的delta值（error值）。注意：对bias也需要求出delta值
for t=1:m
  delta3(t, :) = a3(t, :) - Y(t, :);
  delta2(t, :) = (delta3(t, :) * Theta2(:, :)) .* sigmoidGradient([1 z2(t, :)]);
  
  delta_sum2 = delta_sum2 + (delta3(t, :)' * [1 a2(t, :)]);
  delta_sum1 = delta_sum1 + (delta2(t, 2:end)' * [1 X(t, :)]);
  
endfor

##最后别忘除上数量m
Theta2_grad(:, :) = delta_sum2 / m;
Theta1_grad(:, :) = delta_sum1 / m;
